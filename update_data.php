			...
			if ($account_no_old != $account_no) {
				...
				if ($check->rowCount() > 0) {
					...
					$stmt->execute([
						...
						':building_database_id' => $building_database_id
					]);
				}
			} else {
				$stmt = $pdo->prepare("UPDATE buildings SET account_no = :account_no, building_category = :building_category, building_storey = :building_storey, building_population = :building_population, building_location = :building_location, building_dma_id = :building_dma_id WHERE building_database_id = :building_database_id");
		...
		try {
			...
			if ($pipeline_id_old != $pipeline_id) {
				...
				if ($check->rowCount() > 0) {
					...
					$stmt = $pdo->prepare("UPDATE pipelines SET pipe_id = :pipeline_id, pipeline_dma_id = :pipeline_dma_id, pipeline_diameter = :pipeline_diameter, pipeline_location = :pipeline_location, pipeline_category = :pipeline_category, length = :length WHERE pipeline_database_id = :pipeline_database_id");
					$stmt->execute([
						...
						':length' => $length,
						':pipeline_database_id' => $pipeline_database_id
					]);
				}
			} else {
				$stmt = $pdo->prepare("UPDATE pipelines SET pipe_id = :pipeline_id, pipeline_dma_id = :pipeline_dma_id, pipeline_diameter = :pipeline_diameter, pipeline_location = :pipeline_location, pipeline_category = :pipeline_category, length = :length WHERE pipeline_database_id = :pipeline_database_id");
		...
		try {
			...
			if ($valve_id_old != $valve_id) {
				...
				if ($check->rowCount() > 0) {
					...
					$stmt = $pdo->prepare("UPDATE valves SET valve_id = :valve_id, valve_dma_id = :valve_dma_id, valve_type = :valve_type, valve_diameter = :valve_diameter, valve_location = :valve_location, valve_visibility = :valve_visibility WHERE valve_database_id = :valve_database_id");
					$stmt->execute([
						':valve_id' => $valve_id,
						...
						':valve_database_id' => $valve_database_id
					]);
				}
			} else {
				$stmt = $pdo->prepare("UPDATE valves SET valve_id = :valve_id, valve_dma_id = :valve_dma_id, valve_type = :valve_type, valve_diameter = :valve_diameter, valve_location = :valve_location, valve_visibility = :valve_visibility WHERE valve_database_id = :valve_database_id");
